...
void array2Plot(float *aux, float *glPtr, int num, float *max, int hilo)
{
    ...
    if (index < num)
    {
        ...
        glPtr[idx * 4]     = (index * 2.0 / num) - 1;     // eje x de -1 a 1
        glPtr[idx * 4 + 1] = aux[index] / max[0] * AJUSTE_PLOT - 1;              // eje y de 0 a 2
        glPtr[idx * 4 + 2] = ((index+1) * 2.0 / num) - 1; // eje x de -1 a 1
        glPtr[idx * 4 + 3] = aux[index] / max[0] * AJUSTE_PLOT - 1;              // eje y de 0 a 2

        //printf("glPtr: %d, %f, %f\n", index, glPtr[index*2], glPtr[index*2+1]);
    }

//    if (index == 0 && hilo == 0) printf("\n máximo en plotting: %f\n", max[0]);
}
             ...
             int n, int l, int samples, int pi, float *glPtr)
{
	...
	if (index_X < samples)
	{
       ...
       //     if (hilo == 0) printf("bucles en máximo: %d - num_maximos = %f\n", contador, maximos);
...
//    gpuErrchk(cudaGraphicsGLRegisterBuffer(&res, buf, cudaGraphicsRegisterFlagsNone));
...
{
//    gpuErrchk(cudaGraphicsUnregisterResource((cudaGraphicsResource *) res));
...
{
//    gpuErrchk(cudaGraphicsMapResources(1, (cudaGraphicsResource **) &res));
...
{
//    gpuErrchk(cudaGraphicsUnmapResources(1,(cudaGraphicsResource **) &res));
