...
void OGL_graphic::paintGL()
{
    ...
    {
        if (i < num_casos)
            glColor3f(1.0, float(0.16 * i), 0.0);
        else
            glColor3f(0.0, float(0.16 * (i - num_casos)), 1.0);
...
// ************************************************************************************************
void OGL_graphic::setNum_samples(int value1, int value2)
{
    num_samples = value1;
    num_casos   = value2;
}
...
void OGL_graphic::mouseReleaseEvent(QMouseEvent * event)
{
    ...
    emit ogl_coordinates(xAtPress <= xAtRelease ? xAtPress : xAtRelease,
                         yAtPress,
                         xAtPress <= xAtRelease ? xAtRelease : xAtPress,
                         yAtRelease);
}
