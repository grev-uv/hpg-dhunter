...
void array2Plot(float *aux, float *glPtr, int num, float *max, int hilo)
{
    ...
    if (index < num)
    {
        ...
        glPtr[idx * 4]     = (index * 2.0 / num) - DESPLAZAMIENTO_DIBUJO;     // eje x de -1 a 1
        glPtr[idx * 4 + 1] = aux[index] / max[0] * AJUSTE_PLOT - DESPLAZAMIENTO_DIBUJO;              // eje y de 0 a 2
        glPtr[idx * 4 + 2] = ((index+1) * 2.0 / num) - DESPLAZAMIENTO_DIBUJO; // eje x de -1 a 1
        glPtr[idx * 4 + 3] = aux[index] / max[0] * AJUSTE_PLOT - DESPLAZAMIENTO_DIBUJO;              // eje y de 0 a 2
